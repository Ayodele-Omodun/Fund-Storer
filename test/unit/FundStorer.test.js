...
!developmentChains.includes(network.name) 
    ...
    describe("Deposit", () => {
        ...
        it("sets the time deposited correctly", async () => {
            ...
            assert.equal((block.timestamp / 1000).toFixed(0), (await fundStorer.getTimeDeposited(deployer)).toString())
        })
        it("calculates the time correctly",  async () => {
            ...
            assert.equal(
                ((await fundStorer.getTimeDeposited(deployer)).add(time)).toString() - currentTime,
                 (await fundStorer.getTimeLeft(deployer)).toString()
                )
        })
        it("sets time length correctly", async () => {
            const timeDeposited = ( await fundStorer.getTimeDeposited(deployer))
            assert.equal( (await fundStorer.timeLength(deployer)).toString(), time)
        })
        it("maps the saver's address to the amount sent", async () => {
            const amountStored = await fundStorer.getAmountStored(deployer)
        ...
        it("emits an event after deposit", async () => {
            await expect(fundStorer.deposit(time, {value: amount})).to.emit(fundStorer, "Deposited")
        })
    }) 
    ...
    describe("destroy",  () => {
        it("pays depositers before destruction", async () => {
            const tx1 = fundStorer.deposit(amount)
            await tx1.wait(1)
            const tx2 = fundStorer.destroy()
            await tx2.wait(1)
            const initialSaverBalance = await ethers.provider.getBalance(deployer)
            network.provider.send("evm_increaseTime", [36000000])
            network.provider.send("evm_mine", [])
